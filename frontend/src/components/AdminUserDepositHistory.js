...
const AdminUserDepositHistory = ({ userId, userName, isOpen, onClose }) => {
  ...
  const {
    ...
    queryKey: ['adminUserDeposits', userId, page],
    queryFn: async () => {
      const response = await adminAPI.getUserDeposits(userId, { page, limit });
      return response;
    },
  ...
  const deposits = depositsData?.data?.data || depositsData?.data || [];
  const pagination = depositsData?.data?.pagination || depositsData?.pagination || null;
        ...
        <div className="p-3 sm:p-6 overflow-y-auto max-h-[calc(95vh-120px)] sm:max-h-[calc(90vh-140px)]">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              <div className="text-red-400 mb-4">Failed to load deposits</div>
            ...
            <div className="space-y-3">
              {deposits.map((deposit) => (
                <Card key={deposit.id} className="backdrop-blur-xl bg-white/10 border border-white/20">
                  <div className="p-3">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div className={`text-xs px-2 py-1 rounded-full ${getStatusColor(deposit.status)}`}>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white text-xs">{deposit.network || 'N/A'}</span>
                        </div>
                      </div>
                      <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                        <div className="flex items-center gap-2">
                          <span className="text-gray-300">Deposited:</span>
                          <span className="text-white text-xs">{formatDate(deposit.createdAt)}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-gray-300">Updated:</span>
                          <span className="text-white text-xs">{formatDate(deposit.updatedAt)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {deposit.adminNotes && (
                      <div className="mt-3 p-2 bg-gray-800/50 rounded-lg">
                        ...
                        <div className="text-white text-xs">{deposit.adminNotes}</div>
                      </div>
                    )}
                  </div>
                </Card>
